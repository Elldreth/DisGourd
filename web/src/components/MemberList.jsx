...
// with an owner badge. Clicking another member starts a direct message.
export default function MemberList({ members, currentUser, onMessageUser }) {
  ...
  return (
    <aside className="hidden w-56 flex-col bg-ink-800 lg:flex">
      ...
      <div className="flex-1 space-y-4 overflow-y-auto px-2 py-3">
        ...
        <Group label="Online" people={online} status="online" currentUser={currentUser} onMessageUser={onMessageUser} />
        <Group label="Offline" people={offline} status="offline" currentUser={currentUser} onMessageUser={onMessageUser} />
      </div>
    </aside>
  );
}

function Group({ label, people, status, currentUser, onMessageUser }) {
    ...
    <div>
      ...
      {people.map((p) => {
        ...
        return (
          <button
            key={p.username}
            onClick={() => !isSelf && onMessageUser && onMessageUser(p.username)}
            disabled={isSelf}
            title={isSelf ? 'You' : `Message ${p.username}`}
            className={`flex w-full items-center gap-2 rounded px-2 py-1.5 text-left transition ${
              status === 'offline' ? 'opacity-50' : ''
            } ${isSelf ? 'cursor-default' : 'hover:bg-ink-700/60'}`}
          >
